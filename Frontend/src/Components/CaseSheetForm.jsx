...
`;
	...
	useEffect(() => {
		const fetchSuggestions = async () => {
			...
			const res = await fetch(
				`${API_URL}/api/brain/search?q=${rubricInput}`
			);
	...
	return (
		<form className="case-container" onSubmit={handleSubmit}>
			...
			<section className="case-section">
				...
				<div className="case-form-column">
					...
					<div className="case-section">
						<label className="case-file-input-label">
							Upload Face Image:
						</label>
						<input
							type="file"
						...
						<div className="case-form-column">
							<div className="case-form-group">
								...
								<input
									...
									onChange={(e) =>
										handleChiefComplaintChange(index, e)
									}
			...
			<div className="case-form-group">
				<label className="case-label">
					Rubric Suggestion (type to search)
				</label>
				<input
					className="case-input"
					value={rubricInput}
					...
					<div className="selected-rubrics">
						{selectedRubrics.map((rubric, i) => (
							<span key={i} className="selected-rubric-chip">
								{rubric}
								<button onClick={() => handleRemoveRubric(i)}>
									×
								</button>
							</span>
						))}
						...
						{brainResult && (
							<div className="brain">
								...
								<div>
									...
									<p>
										<strong>Miasm:</strong>{" "}
										{brainResult?.main_remedy?.miasm ||
											"N/A"}
									</p>
									<p>
										<strong>Reason:</strong>{" "}
										{brainResult.main_remedy?.reason ||
											"N/A"}
									</p>

									{brainResult?.main_remedy?.key_symptoms
										?.length > 0 && (
										<>
											<p>
											...
											<ul>
												{brainResult?.main_remedy.key_symptoms.map(
													(s, i) => (
														<li key={i}>{s}</li>
													)
												)}
									...
									{brainResult.next_best_remedies?.length >
										0 && (
										<>
											<h4>Next Best Remedies</h4>
											<ul>
												{brainResult?.next_best_remedies.map(
													(r, i) => (
														<li key={i}>
															<strong>
																{r.name}
								...
								<p>
									...
									caseData.prescription.length > 0
										? caseData.prescription[
												caseData.prescription.length - 1
										  ].remedyName || "N/A"
										: "N/A"}
								</p>
							</div>
